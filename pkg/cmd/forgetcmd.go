...
func (c *Config) runForgetCmd(cmd *cobra.Command, args []string, sourceState *chezmoi.SourceState) error {
	...
	for _, targetRelPath := range targetRelPaths {
		...
		sourceStateOrigin := sourceStateEntry.Origin()
		if _, ok := sourceStateOrigin.(chezmoi.SourceStateOriginAbsPath); !ok {
			c.errorf("warning: %s: cannot forget entry from %s\n", targetRelPath, sourceStateOrigin)
			continue
		}
